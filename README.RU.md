...
Далее — рабочая запись о четырёх патчах CMP50HX в каталоге `patches/cmp50hx/`.
...
текущего ядра, устанавливает их с резервной копией и запускает `depmod`. Он не
пересобирает initramfs и не выгружает загруженный драйвер. В конце он выполняет
проверку карты в работающей системе, если это возможно.

Когда карта работает (проверяющая программа выводит
`PASS_CMP50HX_ISSUE_RATE_AND_COUNTS`), сделайте изменение постоянным после
перезагрузок:
...
частоты: даже явная блокировка диапазона `min,max` приводится к максимуму.
Единственный рычаг, который прошивка принимает со стороны хоста, — жёсткий
потолок частоты, и он опускает карту до **32,4 Вт**.

[`idle-governor/`](idle-governor/README.md) подаёт этот потолок автоматически:
зажимает частоту в простое и снимает ограничение, как только появляется
нагрузка:

| | Простой | Под нагрузкой |
|---|---|---|
| без регулятора | 62–64 Вт | полные частоты |
| с регулятором | **32,4 Вт** | полные частоты, восстановление за один опрос |

Примерно **−29 Вт на каждую простаивающую карту**. Компонент необязательный,
...
- **Совместимо с регулятором простоя:** при применении блокировки частоты
  автоматически пишется drop-in `CMP_LOAD_CLOCK`, поэтому регулятор
  восстанавливает настроенную частоту, а не сбрасывает её.
...
4. В BAR0 `0x8872C` записывается LTSSM `6`, значение читается обратно, затем
   выполняется ожидание 50 мс.
5. На GPU и мосте устанавливается `PCI_EXP_LNKCTL2_TLS_5_0GT`, на мосте
   устанавливается `PCI_EXP_LNKCTL_RL`, после чего состояние соединения GPU
   опрашивается до 20 раз с интервалом 100 мс.
6. Для успеха активный класс состояния соединения должен быть не ниже
   `PCI_EXP_LNKSTA_CLS_5_0GB`; драйвер записывает в журнал
   `CMP50_GEN2: RETRAIN_PASS`. Ошибки возможностей и тайм-ауты записывают
   отказ и останавливают работу, не делая вид, что соединение изменилось.
